<section class="sliderx w-full relative group transition-all duration-300 overflow-hidden rounded-xl">
    <div class="owl-carousel rounded-xl h-48 lg:h-[400px] z-10 relative w-full overflow-hidden">
        @foreach ($slider_gambar['gambar'] as $data)
            @php
                $img = $slider_gambar['lokasi'] . 'sedang_' . $data['gambar'];
            @endphp
            @if (is_file($img))
                <figure class="h-48 lg:h-[400px] w-full overflow-hidden rounded-xl">
                    <img src="{{ base_url($img) }}" alt="{{ $data['judul'] }}" class="max-w-full w-full h-48 lg:h-[400px] object-cover rounded-xl">
